...
\section{Tests de hipótesis}
    ...
    \subsection{Tests de Neyman-Pearson}
        ...
        En el ejemplo anterior hemos realizado el test para un determinado valor de $\alpha$, obteniendo una regla sencilla para aceptar o rechazar la hipótesis nula. No obstante, algunas muestras tienen más evidencia en su contra que otras. %Si reducimos el valor de significación del tests, entonces aceptaremos muestras con mayor evidencia en contra de la hipótesis nula.
        Por tanto, es conveniente obtener un indicador de la evidencia que tiene una muestra en contra de la hipótesis nula.

        \begin{definition}
            Supongamos que para múltiples valores de $\alpha$ disponemos de un test de hipótesis de nivel de significación $\alpha$. En este contexto el p-valor de una muestra es el supremo de los valores de $\alpha$ para los cuales se rechazaría la hipótesis nula si utilizamos el test de significación $\alpha$.
        \end{definition}

        A partir del p-valor de una muestra podemos decidir si la aceptamos o no en función de si elegimos una significación superior o inferior. Algunos usuarios de los tests de hipótesis interpretan el p-valor como la probabilidad de que la hipótesis nula sea falsa para la muestra obtenida. Esto es erróneo y en ningún momento deberíamos utilizar esta interpretación. En la Sección \ref{sec:bayes} estudiaremos tests de hipótesis desde el punto de vista bayesiano. En estos tests sí obtendremos probabilidades de que la hipótesis nula sea cierta o no.

        \begin{prop} \label{prop:p-valor}
            Consideramos el contraste de hipótesis dado por $H_0: \theta \in \Theta_0$ y $H_1: \theta \in \Theta_1$. Supongamos que para cada $\alpha$ tenemos un test de tamaño $\alpha$ que rechaza $H_0$ si y solo si $W(\utilde{x}) \ge c_\alpha$, donde $W$ es un estadístico que no depende de $\alpha$ y $c_\alpha$ es una constante que depende $\alpha$. Entonces, el p-valor de una muestra $\utilde{x}$ es
            \[\sup\{P_\theta(W(\utilde{X}) \ge W(\utilde{x})): \theta \in \Theta_0\}.\]
        \end{prop}
        \begin{proof}
        ...
        Bajo las hipótesis de la Proposición \ref{prop:p-valor} el p-valor puede interpretarse como la probabilidad de observar una muestra que sea tan poco favorable a la hipótesis nula como la muestra que se ha obtenido. Esta interpretación puede tomarse como la definción del p-valor en algunos casos. Por ejemplo, esta definición es válida para los tests de Neyman-Pearson, cuyos p-valores pueden escribirse como sigue
        \[P_{\theta_0}\left(\frac{L(\theta_1;\utilde{X})}{L(\theta_0;\utilde{X})} \ge \frac{L(\theta_1;\utilde{x})}{L(\theta_0;\utilde{x})}\right).\]
