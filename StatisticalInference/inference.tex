...
\begin{definition}
    Sean $\alpha, \beta > 0$. Deinimos la distribución $Gamma(x | \alpha, \beta)$ como la distribución que tiene función de densidad
...
\begin{prop}
    La función característica de la distribución $Gamma(\alpha, \beta)$ viene dada por $\varphi_X(t) = (\beta - i t)^{-\alpha}$.
\end{prop}
\begin{proof}
    Basta utilizar el cambio de variable $y = (\beta - it) x$ como sigue
    \[E[e^{itX}] = \frac{\beta^\alpha}{\Gamma(\alpha)} \int_{-\infty}^{\infty} x^{\alpha-1}e^{-(\beta - it) x} dx = \frac{\beta^\alpha}{\Gamma(\alpha) (\beta -it)^\alpha} \int_{-\infty}^{\infty} y^{\alpha-1}e^{-y} dy = \left(\frac{\beta}{\beta -it}\right)^\alpha.\]
\end{proof}
