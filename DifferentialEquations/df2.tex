...
En la Sección \ref{sec:ae} hemos conseguido calcular las imágenes de las soluciones maximales de
EDOs autónomas escalares. Este hecho se ha conseguido gracias al Teorema de Picard-Lindelöf. En este
punto intentamos generalizar algunos de estos argumentos para EDOs arbitrarias.

\begin{proposition} \label{prop:encerrar}
  Supongamos que $f$  Sea $\varphi$ solución maximal de \eqref{eq:pvi}.
\end{proposition}
...
Todos los resultados se enuncian y se prueban para el extremo superior del dominio de una
solución. No obstante, también se pueden establecer de forma análoga para el extremo inferior, hecho
que se deja al lector como ejercicio.
...
\begin{proof}
  ...
  Existe $a > 0$ tal que $\widetilde{\varphi} \colon ]\omega-a, \omega+a[ \to \R^d$ es solución del
  nuevo PVI. La función $\Psi \colon ]\alpha, \omega+a[ \to \R^d$ dada por
  \[\Psi(t) = \begin{cases} \varphi(t) & \text{si } t \le \omega; \\ \widetilde{\varphi}(t) &
      \text{si } t > \omega;\end{cases}\] es una solución de \eqref{eq:pvi} (Lemma
  \ref{lem:concatenar-sols}) y, por tanto, es una prolongación de $\varphi$.
\end{proof}
...
\begin{proof}
  ...
  \[M = \max \{||f(t,x)||: (t,x) \in R_{a,b}(\omega, \xi)\}.\]

  Fijado $0 < \varepsilon < b$, tomamos $\delta > 0$ tal que $\delta < a$ y
  $\delta M < \varepsilon / 2$. Consideramos la región $R_{\delta, \varepsilon}(\omega, \xi)$. Por
  la convergencia de $\{t_n\}$ y $\{\varphi(t_n)\}$ encontramos $m \in \N$ tal que
  $t_m \in ]\omega-\delta, \omega[$ y $\varphi(t_m) \in \overline{\mathrm{B}}(\xi,
  \varepsilon/2)$. Por tanto, tenemos que
  $(t_m, \varphi(t_m)) \in R_{\delta, \varepsilon}(\omega, \xi)$. Vamos a demostrar que
  $(t, \varphi(t)) \in R_{\delta, \varepsilon}(\omega, \xi)$ para todo $t \in [t_m, \omega[$.
  Razonamos por reducción al absurdo. En tal caso mediante un argumento del primer instante existe
  Nótese que para cada $t \in ]t_m, \omega[$ se verifca
  ...
  \[ ||\varphi(t)-\varphi(t_m)|| = ||\int_{t_m}^{t} f(s,\varphi(s)) \diff s|| \le \int_{t_m}^{t}
    ||f(s,\varphi(s))|| \diff s \le M |t-t_m| \le M \delta < \varepsilon/2.\]
  
  Por último, hemos demostrado que
  $||\varphi(t) - \xi|| \le ||\varphi(t_m) - \xi|| + ||\varphi(t)-\varphi(t_m)|| \le \varepsilon / 2
  + \varepsilon/2$. Esto es, $\varphi(t) \in \overline{\mathrm{B}}(\xi, \varepsilon)$ para todo
  $t \in ]t_m, \omega[$. De la arbitrariedad de $\varepsilon$ se deduce que
...
\begin{proof}
  ...
  $(\omega, \xi) \in \mathrm{Fr}(D) \setminus D = \partial D$.
\end{proof}
...
\begin{remark}
  ...
  a $\mathrm{L}_{\omega}(\varphi) = \emptyset$ mientras que la alternativa \ref{item:prolongacion:1}
...
\begin{corollary}
  ...
  $(\omega, \xi) \in \partial D$ y $\{t_n\} \subset ]\alpha, \omega[$ creciente tal que
  $\{t_n\} \to \omega$ y $\{\varphi(t_n)\} \to \xi$.
\end{corollary}
...
\begin{ex}
  ...
  $\R^2$ deducimos que si $\omega < +\infty$, entoces la solución explota en tiempo finito.
\end{ex}
...
\begin{ex} \label{ex:acotar-f} Consideremos el PVI
  \[
    ...
    \end{cases}
  \]
  donde $a,b \in \R$ con $a < b$, $h \in \mathcal{C}(\R)$ y $g \mathcal{C}(]a,b[)$ acotada por
...
\begin{lemma} \label{lem:potencia:explota} Sea $f\colon \R \times \R \to \R$ continua tal que
  existen $m,p, r \in \R^+$ con $p > 1$ verificando $f(t,x) \ge m x^p$ para todo $t \ge t_0, x > r$.
  Sea $\varphi \colon ]\alpha, \omega[ \to \R$ una solución maximal de \eqref{eq:pvi}. Si $x_0$
  entonces, $\omega < +\infty$ y $\lim_{t \to \omega} \varphi(t) + \infty$.
\end{lemma}
\begin{proof}
  Vamos a encontrar una función que es cota inferior de $\varphi$ y tiene una asíntota
  vertical. Tenemos que ... COMPLETAR.
\end{proof}
...
En esta sección intentamos generalizar el Corolario \ref{cor:dominio}. Como motivación considérese
...
\begin{definition}
  Sea $D = ]a,b[ \times R^2$ y $f\colon ]a,b[\times\R^d$. Diremos que $f$ tiene a lo sumo
...
\begin{proof}
  ...
  lineal. Sea $\varphi \colon ]\alpha, \omega[ \to \R^d$ solución de \eqref{eq:pvi}. Veamos que
  $\omega = b$. Supongamos por reducción al absurdo que $\omega < b$. Tenemos que
...
$f\colon D \to \R^d$ una función continua y sea $(t_0, x_0) \in D$. Consideramos el PVI
\eqref{eq:pvi}.
...
\begin{proposition}
  Supongamos que $d = 1$. Sea $\varphi \colon ]\alpha, \omega[ \to \R$ una solución maximal de
