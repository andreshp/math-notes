...
\begin{definition}
  Sea $D \subset \R^d \times \R^m$ y sea $f \colon D \to \R^k$. La función $f$ es
  \emph{lipschitziana respecto de la segunda variable} si existe $M \ge 0$ tal que para cada
  $x,x' \in \R^d$, $y \in \R^m$ con $(x,y) \in D$ y $(x', y) \in D$ se tiene
  \[ ||f(x,y)-f(x',y)|| \le M ||x-x'||. \]
\end{definition}

\begin{definition}
  Sea $D \subset \R^d \times \R^m$ y sea $f \colon D \to \R^k$. La función $f$ es \emph{localmente
    lipschitziana respecto de la segunda variable} o \emph{LL respecto de la segunda variable} si
  para cada $(p,q) \in D$ existe un entorno abierto $U \subset D$ de $(p,q)$ tal que $f$ es
  lipschitziana respecto de la segunda variable en $U$.
\end{definition}

\begin{proposition}
  Sea $D \subset \R^d \times \R^m$ y sea $f \colon D \to \R^k$ continua y derivable respecto de la
  primera variable $x \in \R^{d}$. Si la función $\frac{\partial}{\partial x}f \colon D \to \mathcal{M}_{d,k}(\R)$ es
  continua, entonces $f$ es LL respecto de la segunda variable.
\end{proposition}

\subsubsection{Operador integral de Volterra} \label{sec:eu:pl:volterra}

Fijado una EIV \eqref{eq:volterra} queremos definir un operador $V \colon E \to F$ tal que a la función
$y\colon I \to \R^d$ le asigne la función $V(y)\colon I \to \R^d$ dada por
\begin{equation}
  \label{eq:1}
...
Como consecuencia del Teorema de Picard-Lindelöf y los resultados de la Sección \ref{sec:unicidad-maximal} obtenemos el siguiente corolario.
...
\[\varphi: (\alpha, \omega) \to \mathbb{R}^d \text{ con } -\infty \leq \alpha < t_0 < \omega \leq
  +\infty.\] Algunos autores escriben $(\omega_-, \omega_+)$ en lugar de $(\alpha, \omega)$. No
obstante, la notación que hemos escogido nos facilita la escritura.

Dado $D \subset \mathbb{R} \times \mathbb{R}^d$ y $f : D \to \mathbb{R}^d$ continua y localmente
...
$X(t_0, t_0, x_0) = x_0$. Esto es, la función $\varphi(t) = X(t, t_0, x_0)$ es la solución del PVI
asociado a $(t_0, x_0)$.
