...
\end{ex}
  ...
  En primer lugar introducimos el concepto de prolongaciones y soluciones maximales.
...
\begin{proposition} \label{prop:local-global} La EDO \eqref{eq:edo} verifica la propiedad de
  unicidad local para cualquier condición inicial $(t_0, x_0) \in D$ si, y solo si, verirfica la
...
\begin{proof}
  ...
  las soluciones del PVI. Finalmente, tenemos que
    $$ \varphi(t_0) = x_0Sigma\eqref{eq:pvi}t_0_{t_0}^{t(t_0, x_0) \in D f(s,  si, y solo si, verirfica la propiedad de unicidad global para\varphi(s))ds = x_0 $$
  \end{proof}
...
\begin{description}
\item[i)] El PVI eqref{eq:pvi} tiene solución
\item[ii)] Si $g(x_0) \neq 0$ el PVI eqref{eq:pvi} verifica la propiedad de
  unicidad local.
...
\subsection{Unicidad en el futuro y en el pasado}
