...
El siguiente lema, cuya demotración es una simple comprobación, nos proporciona a partir de una
...
\begin{definition}
  ...
  soluciones que tienen ese segmento como órbita.
\end{definition}
...
\begin{ex}[Ecuación logística] \label{ex:logistica} Sean $\lambda, L > 0$. Consideramos la EDO
  ...
  \[-\infty \rightarrow 0 \leftarrow L \rightarrow +\infty.\]
\end{ex}

\begin{cor}
  Sea $\varphi \in \mathcal{C}^1(]\alpha, \omega[)$ solución maximal de \eqref{eq:edo:ae} tal que
  $\lim_{t \to \omega} \varphi(t) = L \in \R$. Entonces, $p \in \mathrm{Z}_f$.
\end{cor}
\begin{proof}
  Es una consecuencia sencilla de la Proposición \ref{prop:ae:intervalos}.
\end{proof}
...
\begin{ex}[Ecuación logística] \label{ex:logistica:2} Recuperamos el Ejemplo \ref{ex:logistica},
  donde se estudiaba la ecuación logística. Sea $x_0 \in \R$.
  \begin{enumerate}
  \item Si $x_0 \in \{0, L\}$, entonces $]\alpha(x_0), \omega(x_0)[ = \R$.
  \item Si $x_0 < 0$, entonces $\omega(x_0) = +\infty$. Además, $\lim_{t \to +\infty} X(t,x_0) = 0$.
  \item Si $0 < x_0 < L$, entonces $\alpha(x_0) = -\infty$ y $\omega(x_0) = +\infty$. Además,
    $\lim_{t \to -\infty} X(t,x_0) = L$ y $\lim_{t \to +\infty} X(t,x_0) = 0$.
  \item Si $x_0 > L$, entonces $\alpha(x_0) = -\infty$. Además, $\lim_{t \to -\infty} X(t,x_0) = L$. \qedhere
  \end{enumerate}
\end{ex}
...
\begin{definition}
  \label{def:ra}
  Sea $I \subset \R$ intervalo abierto, $f\colon I \to \R$ localmente lipschitziana y $p \in I$ tal
  que $f(p) = 0$. Se define la región de atracción de $p$ como
